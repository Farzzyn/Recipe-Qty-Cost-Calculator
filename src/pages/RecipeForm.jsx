            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="md:col-span-1">
                ...
                <input 
                  ...
                  className="w-full bg-slate-900/50 border border-slate-700/50 rounded-xl py-3 px-4 text-slate-100 focus:outline-none focus:border-emerald-500 transition-colors"
              ...
              <div>
                ...
                <select 
                  ...
                  onChange={handleRecipeChange}
                  className="w-full bg-slate-900/50 border border-slate-700/50 rounded-xl py-3 px-4 text-slate-100 focus:outline-none focus:border-emerald-500 transition-colors appearance-none"
                >
          ...
          <div className="glass-card rounded-2xl p-6 md:p-8 overflow-hidden">
            <div className="flex justify-between items-center mb-6">
              ...
              <button 
                ...
                className="flex items-center gap-2 text-sm font-medium text-emerald-400 hover:text-emerald-300 bg-emerald-500/10 hover:bg-emerald-500/20 px-3 py-1.5 rounded-lg transition-colors border border-emerald-500/20"
              >
                    ...
                    <div key={ing.id || index} className="grid grid-cols-12 gap-3 items-center bg-slate-900/30 p-2 rounded-xl border border-slate-800/50">
                      <div className="col-span-3">
                        <input 
                          ...
                          className="w-full bg-slate-800/50 border border-slate-700/50 rounded-lg py-2 px-3 text-slate-100 text-sm focus:outline-none focus:border-emerald-500 transition-colors"
                        />
                      </div>
                      ...
                      <div className="col-span-2 flex gap-1">
                        <input 
                          ...
                          placeholder="Qty"
                          className="w-full min-w-0 bg-slate-800/50 border border-slate-700/50 rounded-lg py-2 px-2 text-slate-100 text-sm focus:outline-none focus:border-emerald-500 transition-colors"
                        />
                        <select 
                          value={ing.unit}
                          onChange={(e) => handleIngredientChange(index, 'unit', e.target.value)}
                          className="w-16 bg-slate-800/50 border border-slate-700/50 rounded-lg py-2 px-1 text-slate-100 text-xs focus:outline-none focus:border-emerald-500"
                        >
                          {UNITS.map(u => <option key={u} value={u} className="bg-slate-900 text-slate-100">{u}</option>)}
                        </select>
                      </div>
            ...
            <div className="mt-4 p-4 bg-emerald-500/5 border border-emerald-500/10 rounded-xl text-sm text-slate-400">
              <span className="text-emerald-400 font-medium">Tip:</span> If you don't need cost tracking for an ingredient, leave the purchase quantity and price fields blank.
            </div>
          </div>
        </div>
        ...
        <div className="fixed bottom-0 left-0 right-0 md:left-64 p-4 bg-slate-950/80 backdrop-blur-xl border-t border-slate-800 flex justify-end z-20">
          <div className="flex gap-4 max-w-5xl w-full mx-auto justify-end">
            ...
            <button 
              type="submit"
              disabled={loading}
              className="flex items-center gap-2 px-8 py-2.5 rounded-xl font-semibold text-slate-950 bg-emerald-500 hover:bg-emerald-400 transition-colors shadow-[0_0_20px_rgba(16,185,129,0.3)] disabled:opacity-70"
            >
